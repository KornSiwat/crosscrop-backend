...
sendResponse :: ToJSON a
             => Status
             -> Either Error a
             -> Handler Value
sendResponse status eitherPresenter
    = case eitherPresenter of
        Right presenter -> sendSuccessResponse status presenter
        Left error      -> sendErrorResponse error

sendSuccessResponse :: ToJSON a
                    => Status
                    -> a
                    -> Handler Value
sendSuccessResponse status presenter
    = sendResponseStatus status (toJSON presenter)
